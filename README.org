...
For each problem there is a related file with the suffix ~_adhoc~ or ~naive~. For example, to benchmark ~display::display()~ there is a file ~ctor.cpp~ and another file ~ctor_adhoc.cpp~. 

The benchmarks can be run through the script ~benchmarks/adhoc/make_check_size.sh~. The performance space from an adhoc or naive solution shouldn't be better than the att85 solution.
